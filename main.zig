...
fn fetchDayData(allocator: std.mem.Allocator, station_id: []const u8, date: []const u8) ![]u8 {
    ...
    if (readCache(allocator, station_id, date)) |cached_window| {
        if (isValidTideWindow(&cached_window)) {
            return determineCurrentStatus(allocator, &cached_window);
        }
        // Cache exists but invalid - clear and fetch fresh
    ...
    // Fetch fresh data (both cache miss and invalid cache paths)
    ...
    if (isValidTideWindow(&window)) {
        ...
        // Fresh data is also invalid - clear cache to prevent repeated failures
        try clearCache(allocator);
